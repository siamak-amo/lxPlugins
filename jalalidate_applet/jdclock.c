 ...
 *               2026 Ahmad <edu.siamak@gmail.com>
 *
 * This file is a part of lxPlugin project,
 * and is based on dclock.c from LXPanel project.
 *
...
typedef struct
{
  ...
  enum
    {
      /* Experimenting to determine interval, waiting for first change */
      AWAITING_FIRST_CHANGE,
      /* Experimenting to determine interval, waiting for second change */
      AWAITING_SECOND_CHANGE,
      /* Determined that one second interval is necessary */
      ONE_SECOND_INTERVAL,
      /* Determined that one minute interval is sufficient */
      ONE_MINUTE_INTERVAL,
    } expiration_interval;
...
static GtkWidget *
dclock_create_calendar (DClockPlugin * dc)
{
  /* Create a new window. */
...
static gboolean
dclock_button_press_event (GtkWidget * widget, GdkEventButton * evt, LXPanel * panel)
{
  DClockPlugin * dc;
  (void) panel;
...
/* Set the timer. */
static void
dclock_timer_set (DClockPlugin * dc, struct timeval *current_time)
{
...
static inline int
shrink_percentage (char *buffer, int c)
{
  int Ccount = 0;

  for (int r=0, w=0; buffer[w] != '\0'; ++r, ++w)
    {
      if (buffer[r] == c && r > 0 && buffer[r-1] == '%')
        ++r, ++Ccount;
      if (r != w)
        buffer[w] = buffer[r];
    }
...
static gboolean
dclock_update_display (DClockPlugin * dc)
{
  ...
  char _clock_value[64];
  char _tooltip_value[64];
  _clock_value[0] = '\0';
  if (dc->clock_format != NULL)
    strftime(_clock_value, sizeof(_clock_value), dc->clock_format, current_time);
  _tooltip_value[0] = '\0';
  if (dc->tooltip_format != NULL)
    strftime(_tooltip_value, sizeof(_tooltip_value), dc->tooltip_format, current_time);

  /* Now, it also might have %JX which we will shrink it down to %X and pass it
     to the Jalali jstrftime() to make the final output. */
  char clock_value[128];
  clock_value[0] = '\0';
  char tooltip_value[128];
  tooltip_value[0] = '\0';

  struct jtm * Jcurrent_time;
  Jcurrent_time = jlocaltime (&now.tv_sec);

  if (_clock_value[0] != '\0')
    {
      shrink_percentage (_clock_value, 'J');
      jstrftime (clock_value, sizeof(clock_value), _clock_value, Jcurrent_time);
    }
  if (_tooltip_value[0] != '\0')
    {
      shrink_percentage (_tooltip_value, 'J');
      jstrftime (tooltip_value, sizeof(clock_value), _tooltip_value, Jcurrent_time);
    }
...
/* Plugin constructor. */
static GtkWidget *
dclock_constructor (LXPanel *panel, config_setting_t *settings)
{
...
/* Plugin destructor. */
static void
dclock_destructor (gpointer user_data)
{
...
static gboolean
dclock_apply_configuration (gpointer user_data)
{
...
static GtkWidget *
dclock_configure (LXPanel *panel, GtkWidget *p)
{
    DClockPlugin * dc = lxpanel_plugin_get_data(p);
    return
      lxpanel_generic_config_dlg (_("Jalali Digital Clock"), panel,
                                  dclock_apply_configuration, p,
        ...
        _("Format codes: man 3 strftime; %n for line break; "
          "%Jx to pass %x to jstrftime instead"), NULL, CONF_TYPE_TRIM,
...
static void
dclock_on_panel_reconfigured (LXPanel *panel, GtkWidget *p)
{
  (void) panel;
  dclock_apply_configuration(p);
}
...
FM_DEFINE_MODULE (lxpanel_gtk, jdclock);
